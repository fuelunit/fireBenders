--Function
local DifNPC = true
local StationaryNPC = game.workspace.ScriptAttachedObjects.NPCS.NPC_3.Button.Red.Position
local debounce = true
...
local list = {"Lovely, You got the coffee", "Here take this useless crowbar", "If you have more coffee please bring it"} 
local NPCname = ("Steve")
		...
		--print (player.Name)
				...
				local speechgui = game.Players[player.Name].PlayerGui.SpeechUI
					...
					if NPC < 5 and debounce2 == true then
						...
						workspace.ScriptAttachedObjects.Objects.Crowbar:Remove()
						local changedialogue = game.Players[player.Name].PlayerScripts
						wait(1)
							changedialogue["NPC scripts"]["NPC 3 [Steve]"]["NPC 3.1 [Steve]"].Disabled = true
							changedialogue["NPC scripts"]["NPC 3 [Steve]"]["NPC 3.2 [Steve]"].Disabled = false
							
							changedialogue["Objects"]["Basement"].Disabled = true
							changedialogue["Objects"]["Basement"]["Basement 1.1"].Disabled = false
						Control:Enable()
						wait(10)
						debounce2 = true
